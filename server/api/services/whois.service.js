  ...
  whois(name) {
    ...
    return EveService.search(name, true, 'character').then(searchResult => EveService.characters(searchResult.character[0]).then(character => {
      ...
      if (character.alliance_id) {
        l.info('has alliance');
        output.alliance = { alliance_id: character.alliance_id };
      ...
      output.corporation = { corporation_id: character.corporation_id };
      ...
      // Get zkill info.
      output.killboard = { url: `https://zkillboard.com/character/${id}` };
